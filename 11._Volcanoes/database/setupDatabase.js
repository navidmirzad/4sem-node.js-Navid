...
if (isDeleteMode) {
  await db.run(
    `INSERT INTO volcanoes (name, is_active, type) VALUES ('Mount St. Helens', TRUE, 'Caldera')`
  );
  await db.run(
    `INSERT INTO volcanoes (name, is_active, type) VALUES ('Krakatoa', FALSE, 'Super Volcanoe')`
  );
  await db.run(
    `INSERT INTO villages (name, volcanoe_id) VALUES ('Village 1', 1)`
  );
  await db.run(
    `INSERT INTO villages (name, volcanoe_id) VALUES ('Village 2', 1)`
  );
}
